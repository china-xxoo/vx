...
const CORS = {
  ...
  "Access-Control-Allow-Headers": "Content-Type,Authorization,X-Device-Id,X-Push-Secret",
...
function jsonToBase64Url(data) {
  return bytesToBase64Url(enc.encode(JSON.stringify(data)));
}

function p256PublicKeyFromJwk(jwk) {
...
function derToRawEcdsa(signature) {
...
function cleanPushRooms(value) {
  const list = Array.isArray(value) ? value : [];
  return [...new Set(list.map(cleanRoomNo).filter(no => no.length === ROOM_NO_DIGITS))].slice(0, 200);
}

function cleanEndpoint(value) {
  const endpoint = String(value || "").trim();
  return /^https:\/\/.+/i.test(endpoint) ? endpoint.slice(0, 500) : "";
}
...
function vapidPublicKey(env) {
  return env.VAPID_PUBLIC_KEY || p256PublicKeyFromJwk(vapidJwk(env));
}

async function vapidHeaders(endpoint, env) {
  const jwk = vapidJwk(env);
  if (!jwk?.d || !jwk?.x || !jwk?.y) throw new Error("Missing VAPID_PRIVATE_JWK");
  const audience = new URL(endpoint).origin;
  const subject = env.VAPID_SUBJECT || "mailto:admin@example.com";
  const exp = Math.floor(Date.now() / 1000) + 12 * 60 * 60;
  const token = jsonToBase64Url({ typ: "JWT", alg: "ES256" }) + "." + jsonToBase64Url({ aud: audience, exp, sub: subject });
  ...
  const signature = await crypto.subtle.sign({ name: "ECDSA", hash: "SHA-256" }, key, enc.encode(token));
  return {
    "TTL": "2419200",
    "Authorization": "WebPush " + token + "." + bytesToBase64Url(derToRawEcdsa(signature)),
    "Crypto-Key": "p256ecdsa=" + vapidPublicKey(env)
  };
}
...
async function handlePushPublicKey(env) {
  ...
  return json({ publicKey, enabled: !!publicKey });
}

async function handlePushSubscribe(request, env) {
  if (!requirePushSecret(request, env)) return json({ error: "无权限" }, 403);
  const body = await readJson(request);
  const subscription = body.subscription || {};
  const endpoint = cleanEndpoint(subscription.endpoint);
  if (!endpoint) return json({ error: "订阅无效" }, 400);
  const keys = subscription.keys || {};
  const p256dh = String(keys.p256dh || "").slice(0, 200);
  const auth = String(keys.auth || "").slice(0, 80);
  const rooms = cleanPushRooms(body.rooms);
  const userId = String(body.userId || "").slice(0, 80);
  const deviceId = cleanDeviceId(body.deviceId);
  const stamp = now();
  await env.DB.prepare(
    "INSERT INTO push_subscriptions (endpoint, p256dh, auth, user_id, device_id, rooms, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT(endpoint) DO UPDATE SET p256dh = excluded.p256dh, auth = excluded.auth, user_id = excluded.user_id, device_id = excluded.device_id, rooms = excluded.rooms, updated_at = excluded.updated_at"
  ).bind(endpoint, p256dh, auth, userId, deviceId, JSON.stringify(rooms), stamp, stamp).run();
  return json({ ok: true });
}
...
async function handlePushNotify(request, env) {
  if (!requirePushSecret(request, env)) return json({ error: "无权限" }, 403);
  const body = await readJson(request);
  const roomNo = cleanRoomNo(body.roomNo);
  if (roomNo.length !== ROOM_NO_DIGITS) return json({ error: "房间无效" }, 400);
  const sender = String(body.sender || "").slice(0, 80);
  const result = await env.DB.prepare("SELECT endpoint, user_id, device_id, rooms FROM push_subscriptions WHERE updated_at > ? ORDER BY updated_at DESC LIMIT 1000")
    .bind(now() - 1000 * 60 * 60 * 24 * 120)
    .all();
  let sent = 0;
  let removed = 0;
  for (const item of result.results || []) {
    if (sender && (item.user_id === sender || item.device_id === sender)) continue;
    let rooms = [];
    try {
    ...
    if (!rooms.includes(roomNo)) continue;
    try {
      const response = await sendPush(item, env);
      if (response.status === 404 || response.status === 410) {
        await env.DB.prepare("DELETE FROM push_subscriptions WHERE endpoint = ?").bind(item.endpoint).run();
        removed += 1;
      } else if (response.ok) {
        sent += 1;
      }
...
async function route(request, env) {
  ...
  if (request.method === "GET" && url.pathname === "/push/public-key") return handlePushPublicKey(env);
  if (request.method === "POST" && url.pathname === "/push/subscribe") return handlePushSubscribe(request, env);
  if (request.method === "POST" && url.pathname === "/push/unsubscribe") return handlePushUnsubscribe(request, env);
  if (request.method === "POST" && url.pathname === "/push/notify") return handlePushNotify(request, env);
